...
document.querySelector("#run").addEventListener("click", async () => {
  try {
    ...
    const [input, expected] = await Promise.all([
      fetch("../../fixtures/micro-wasm/simple.input.json").then((response) => response.text()),
      fetch("../../fixtures/micro-wasm/simple.expected.json").then((response) => response.json()),
    ]);
    const actual = JSON.parse(calculate_timeline(input));
    const matches = JSON.stringify(actual) === JSON.stringify(expected);
    status.className = matches ? "ok" : "error";
    status.textContent = matches
      ? `PASS — Native fixture matched (initialization ${(initialized - started).toFixed(3)} ms)`
