...
pub async fn export(
    ...
    let result = exporter.export(ExportRequest {
        project: &project,
    ...
    if json_output {
        println!("{}", serde_json::to_string_pretty(&serde_json::json!({"ok":true,"format":format,"output":result.output,"warnings":result.warnings}))?);
    } else {
        println!("Exported {}", result.output.display());
        for warning in result.warnings { println!("warning: {warning}"); }
    }
    Ok(ExitCode::SUCCESS)
}
