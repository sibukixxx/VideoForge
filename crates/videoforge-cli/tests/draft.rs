...
fn review_export_refuses_stale_hash_and_overwrite() {
    ...
    std::fs::write(dir.path().join("brief.json"), include_str!("../../../fixtures/draft/brief.json")).unwrap();
    std::fs::write(dir.path().join("response.json"), include_str!("../../../fixtures/draft/response.json")).unwrap();
    let run = |args: &[&str]| {
    ...
    let export = |hash: &str, out: &str| run(&[
        "draft", "export", "brief.json", "response.json", "--reviewed-hash", hash,
        "--reviewer", "tester", "--out", out,
    ]);
    ...
    assert!(!export(hash, "scripts/draft.md").status.success());
    assert_eq!(original, std::fs::read(dir.path().join("scripts/draft.md")).unwrap());
    let brief_path = dir.path().join("brief.json");
    let changed = std::fs::read_to_string(&brief_path).unwrap().replace("初心者", "経験者");
