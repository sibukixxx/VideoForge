...
/// background color, and the renderer's own identity (so switching renderer
/// implementations, or a future FFmpeg-version-aware id, never replays a
/// stale render).
...
mod tests {
    ...
    fn same_inputs_produce_the_same_fingerprint() {
        let a = preview_fingerprint("{}", None, "#000000", "ffmpeg");
        let b = preview_fingerprint("{}", None, "#000000", "ffmpeg");
        assert_eq!(a, b);
    }

    #[test]
    fn project_json_change_invalidates() {
        let a = preview_fingerprint("{\"a\":1}", None, "#000000", "ffmpeg");
        let b = preview_fingerprint("{\"a\":2}", None, "#000000", "ffmpeg");
        assert_ne!(a, b);
    }
    ...
    fn background_color_change_invalidates() {
        let a = preview_fingerprint("{}", None, "#000000", "ffmpeg");
        let b = preview_fingerprint("{}", None, "#ffffff", "ffmpeg");
        assert_ne!(a, b);
    }
    ...
    fn renderer_identity_change_invalidates() {
        let a = preview_fingerprint("{}", None, "#000000", "ffmpeg");
        let b = preview_fingerprint("{}", None, "#000000", "ffmpeg-v2");
        assert_ne!(a, b);
    }
    ...
    fn font_file_content_change_invalidates_even_with_the_same_path() {
        ...
        let a = preview_fingerprint("{}", Some(&font), "#000000", "ffmpeg");
        std::fs::write(&font, b"version 2 (different bytes)").unwrap();
        let b = preview_fingerprint("{}", Some(&font), "#000000", "ffmpeg");
        assert_ne!(a, b, "same path, different bytes, must not collide");
    }

    #[test]
    fn missing_font_is_distinct_from_no_font() {
        let with_none = preview_fingerprint("{}", None, "#000000", "ffmpeg");
