...
fn grounded_draft_and_failure_cases() {
    ...
    let mut brief: Brief =
        serde_json::from_str(include_str!("../../../fixtures/draft/brief.json")).unwrap();
    let mut candidate: Draft =
        serde_json::from_str(include_str!("../../../fixtures/draft/response.json")).unwrap();
    ...
    assert!(draft::prompt(&brief, &cfg)
        .unwrap()
        .contains("UNTRUSTED_INPUT_JSON"));

    candidate.dialogues[0].evidence[0].quote = "invented".into();
    assert!(
        !draft::check(&brief, &candidate, &cfg, &ws)
            .unwrap()
            .structurally_valid
    );
